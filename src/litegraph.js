        ...
        for (var i = 0, l = this._nodes.length; i < l; ++i) {
            ...
            if (node.inputs) {
                for (var j = 0, l2 = node.inputs.length; j < l2; j++) {
                    if (node.inputs[j] && hasLinkInObj(node.inputs[j])) {
                        num += 1;
                    }
                }
            }
...
*/
    ...
    LGraphNode.prototype.configure = function (info) {
        ...
        if (this.inputs) {
            for (var i = 0; i < this.inputs.length; ++i) {
                ...
                if (hasLinkInObj(input)) {
                    for (var j = 0; j < input.links.length; ++j) {
                        var link_info = this.graph ? this.graph.links[input.links[j]] : null;
    ...
    LGraphNode.prototype.getInputData = function (slot, force_update) {
        ...
        if (slot >= this.inputs.length || !hasLinkInObj(this.inputs[slot])) {
            return;
        }
    ...
    LGraphNode.prototype.isInputConnected = function (slot) {
        ...
        return slot < this.inputs.length && hasLinkInObj(this.inputs[slot]);
    };
    ...
    LGraphNode.prototype.getInputLink = function (slot) {
        ...
        if (slot < this.inputs.length) {
            ...
            if (hasLinkInObj(slot_info)) {
                return this.graph.links[slot_info.links[0]];
            }
        }
        return null;
    };
    ...
    LGraphNode.prototype.getInputOrProperty = function (name) {
        ...
        for (var i = 0, l = this.inputs.length; i < l; ++i) {
            ...
            if (name == input_info.name && hasLinkInObj(input_info)) {
    ...
    function hasLinkInObj(obj) {
        return obj && obj.links && obj.links.length > 0;
    }

    function removeLinkInObjWithID(obj, id) {
    ...
    LGraphNode.prototype.addInput = function (name, type, extra_info) {
        type = type || 0;
        var input = { name: name, type: type, link: null, links: [] };
        if (extra_info) {
            for (var i in extra_info) {
    ...
    LGraphNode.prototype.addInputs = function (array) {
        for (var i = 0; i < array.length; ++i) {
            var info = array[i];
            var o = { name: info[0], type: info[1], link: [] };
            if (array[2]) {
                for (var j in info[2]) {
                    o[j] = info[2][j];
                }
    ...
    LGraphNode.prototype.removeInput = function (slot) {
        ...
        for (var i = slot; i < this.inputs.length; ++i) {
            ...
            if (!hasLinkInObj(this.inputs[i])) {
                continue;
            }
            var link = this.graph.links[this.inputs[i].links[0]];
            if (!link) {
                continue;
            }
            link.target_slot -= 1;
    ...
    */
    ...
    LGraphNode.prototype.connect = function (slot, target_node, target_slot) {
        ...
        // if (target_node.inputs[target_slot] && hasLinkInObj(target_node.inputs[target_slot])) {
        ...
        //connect in input
        if (target_node.inputs[target_slot].links == null) {
        ...
        if (target_node) {
            ...
            for (var i = 0, l = output.links.length; i < l; i++) {
                ...
                if (link_info.target_id == target_node.id) {
                    ...
                    removeLinkInObjWithID(input, link_id);
    ...
    LGraphNode.prototype.disconnectInput = function (slot) {
        ...
        for (let ii = 0; ii < this.inputs[slot].links.length; ii++) {
            let link_id = this.inputs[slot].links[ii];
            //remove other side
            var link_info = this.graph.links[link_id];
            if (link_info) {
        ...
        this.inputs[slot].links.length = 0;
    ...
    */
        ...
        for (var i = 0; i < selected_nodes_array.length; ++i) {
            ...
            if (node.inputs && node.inputs.length) {
                for (var j = 0; j < node.inputs.length; ++j) {
                    ...
                    for (let k = 0; k < input.links.length; j++) {
    ...
    LGraphCanvas.prototype.selectNodes = function (nodes, add_to_current_selection) {
        ...
        for (var i in nodes) {
            ...
            if (node.inputs) {
                for (var j = 0; j < node.inputs.length; ++j) {
                    for (let k = 0; k < node.inputs[j].links.length; k++) {
                        this.highlighted_links[node.inputs[j].links[k]] = true;
                    }
                }
            }
            if (node.outputs) {
    ...
    LGraphCanvas.prototype.deselectNode = function (node) {
        ...
        if (node.inputs) {
            for (var i = 0; i < node.inputs.length; ++i) {
                for (let j = 0; j < node.inputs[i].links.length; j++) {
                    delete this.highlighted_links[node.inputs[i].links[j]];
                }
            }
        }
        if (node.outputs) {
    ...
    LGraphCanvas.prototype.deleteSelectedNodes = function () {
        ...
        for (var i in this.selected_nodes) {
            ...
            if (node.inputs && node.inputs.length && node.outputs && node.outputs.length && LiteGraph.isValidConnection(node.inputs[0].type, node.outputs[0].type) && hasLinkInObj(node.inputs[0]) && node.outputs[0].links && node.outputs[0].links.length) {
